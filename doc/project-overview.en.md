...
- Store versioned assets in MinIO using `app/version` paths.
- Provide asset read APIs for CDN / frontend clients.
- Keep at most 15 active versions (`success`) per `app`.
...
3. Service extracts files and uploads each file to MinIO under `/{app}/{version}/{file}`.
4. Service inserts a release document into MongoDB with `status = success`.
5. Service performs rotation:
   - Query releases with same `app` and `status=success` (newest first).
...
1. CDN/browser calls `GET /assets/:app/:version/*path`.
...
1. Frontend calls `GET /api/v1/apps/:app/versions` (no token required).
...
- `401` invalid token
- `409` duplicate `(app_name, version)`
- `500` internal error
...
- `storage_prefix` (string) - e.g. `/my-app/1.2.3/`
...
- Example: `/my-app/1.2.3/index.html`
